    ...
    <div className="min-h-screen pt-20 pb-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        >
          ...
          >
            {'>'} LEVEL_3_DASHBOARD.EXE
          </h1>
          <p className="text-xl text-zinc-400 max-w-2xl mx-auto font-mono">
            [INITIALIZING...] Dashboard Premium Level 3 - Silver • {commissionRate}% Comissão
          </p>
        </motion.div>
        ...
        >
          <h3 className="text-2xl font-bold mb-6 font-mono text-center" style={{ color: '#ffffff' }}>
            Seu Link Premium Level 3 - Silver
          </h3>
          ...
          {activeTab === 'overview' && (
            ...
            >
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                {[
                  { label: 'Total de Cliques', value: stats.totalClicks.toLocaleString(), icon: MousePointer, color: '#3ecbff' },
                  { label: 'Cadastros', value: stats.totalSignups.toString(), icon: Users, color: '#00ccff' },
                  { label: 'Depósitos', value: stats.totalDeposits.toString(), icon: CreditCard, color: '#0099ff' },
                  { label: 'Ganhos da Semana', value: `$${stats.weeklyEarnings.toFixed(2)}`, icon: DollarSign, color: '#0066ff' },
                ].map((stat, index) => {
              ...
              >
                <h4 className="text-2xl font-bold mb-6 font-mono text-center" style={{ color: '#ffffff' }}>
                  Resumo Level 3 - Silver
                </h4>
                
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="text-center p-4 rounded-lg" style={{ background: 'rgba(62, 203, 255, 0.1)' }}>
                    <div className="text-3xl font-bold mb-2" style={{ color: '#3ecbff' }}>
                      ${stats.totalEarnings.toFixed(2)}
                    </div>
                    <p className="text-sm text-zinc-400 font-mono">Total Acumulado</p>
                  </div>
                  
                  <div className="text-center p-4 rounded-lg" style={{ background: 'rgba(0, 204, 255, 0.1)' }}>
                    <div className="text-3xl font-bold mb-2" style={{ color: '#00ccff' }}>
                      {stats.conversionRate}%
                    </div>
                    <p className="text-sm text-zinc-400 font-mono">Taxa de Conversão</p>
                  </div>
                  
                  <div className="text-center p-4 rounded-lg" style={{ background: 'rgba(0, 153, 255, 0.1)' }}>
                    <div className="text-3xl font-bold mb-2" style={{ color: '#0099ff' }}>
                      {commissionRate}%
                    </div>
                    <p className="text-sm text-zinc-400 font-mono">Sua Comissão Level 3</p>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          {activeTab === 'analytics' && (
            ...
            >
              <h4 className="text-2xl font-bold font-mono text-center" style={{ color: '#ffffff' }}>
                Analytics Level 3 (Últimos 7 dias)
              </h4>
                    ...
                    <tbody>
                      {stats.dailyData.map((d, i) => (
                        <tr key={i} className="border-b" style={{ borderColor: 'rgba(255, 255, 255, 0.1)' }}>
                          ...
                          <td className="py-3 font-mono" style={{ color: '#3ecbff' }}>{d.clicks}</td>
                          <td className="py-3 font-mono" style={{ color: '#00ccff' }}>{d.signups}</td>
                          <td className="py-3 font-mono" style={{ color: '#0099ff' }}>{d.deposits}</td>
                          <td className="py-3 font-mono" style={{ color: '#0066ff' }}>${d.earnings.toFixed(2)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  ...
                  {[
                    ...
                    >
                      <div className="flex items-center space-x-4">
                        ...
                        >
                          <Wallet className="w-6 h-6" style={{ color: '#3ecbff' }} />
                        </div>
                        <div>
                      ...
                      <div className="text-right">
                        <div className="text-xl font-bold" style={{ color: '#3ecbff' }}>
                          {referral.earnings}
                        </div>
                        <div className="text-sm text-zinc-400 font-mono">
