/*
  # Fix affiliate dashboard RPC to use wallet_address for ticket_purchases

  1. Changes
    - Update `get_affiliate_dashboard_stats` to join ticket_purchases via
      wallet_address from the referred user (users table) instead of user_id,
      since the frontend inserts ticket_purchases with wallet_address only
      (user_id is null).
    - This fixes the total_tickets and weekly_tickets counters showing 0.
...
BEGIN
  ...
  SELECT
    ...
    COALESCE((
      ...
      JOIN affiliates a ON a.id = r.referrer_affiliate_id
      JOIN users aff_u ON aff_u.id = a.user_id
      WHERE aff_u.wallet_address = p_wallet
    ), 0) as total_tickets,
    COALESCE((
      SELECT SUM(tp.quantity)::bigint
      FROM ticket_purchases tp
