/*
  # Fix countdown timer sync with actual claim release schedule

  1. Problem
    - `get_affiliate_dashboard_stats` calculates `next_release_timestamp` using
      `date_trunc('week', now()) + interval '2 days' + interval '7 days'`
      which yields Wednesday 00:00:00 UTC
    - The actual claim availability gate (`is_affiliate_claim_available`) uses
      `get_wednesday_release_timestamp()` which yields Wednesday 23:59:59 UTC
    - Result: the dashboard countdown reaches zero ~24 hours before claims
      actually become available, confusing users

  2. Solution
    - Replace the `date_trunc`-based calculation with a call to the canonical
      `get_next_affiliate_release()` function, ensuring the countdown timer
      and the claim availability gate use the exact same timestamp
    - No data changes, only the display timestamp is corrected
*/
