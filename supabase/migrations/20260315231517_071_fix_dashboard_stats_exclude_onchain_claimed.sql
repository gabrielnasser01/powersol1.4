/*
  # Fix dashboard stats to exclude on-chain claimed weeks from pending claimable

  1. Changes
    - Updated `pending_claimable_lamports` calculation in `get_affiliate_dashboard_stats`
      to exclude weeks that already have an on-chain claim record
    - Updated `weekly_earned_lamports` to use the current week's actual value correctly

  2. Why
    - Weeks that were claimed on-chain but whose `is_claimed` flag was not updated
      were still counted as claimable, inflating the displayed pending amount
*/
