/*
  # Fix week_number calculation in commission trigger

  1. Problem
    - The trigger `process_affiliate_commission_on_purchase()` calculates week_number as:
      `floor(extract(epoch from v_now) / 604800)` (epoch weeks starting Thursday)
    - The function `calculate_week_number()` uses:
      `(unix_ts - 345600) / 604800` (epoch weeks starting Monday)
    - This mismatch causes earnings from Thursday-Sunday to be assigned to wrong weeks
      in the accumulator, resulting in incorrect unclaimed/pending amounts
    - 64 earnings across 2 wallets (10.275 SOL total) have mismatched week numbers

  2. Fix
    - Update the trigger to use `calculate_week_number(v_now)` instead of raw epoch division
    - This ensures consistency between the trigger, backfill migrations, and all RPCs

  3. Important Notes
    - Existing accumulator data will be fixed in a separate step
    - All future earnings will use the correct Monday-aligned week number
*/
