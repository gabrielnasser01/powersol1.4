/*
  # Fix trigger to skip already-claimed weeks in accumulator

  1. Problem
    - Migration 100 fixed the week calculation to use calculate_week_number()
    - But if a week has already been claimed on-chain, new earnings for the same
      calendar week must NOT be added to the claimed row
    - The on-chain claim uses week_number as the nonce, so the next accumulation
      must use the next available (unclaimed) week_number

  2. Fix
    - After calculating v_week_number via calculate_week_number(), check if that
      week already exists and is_claimed=true for this wallet
    - If so, use v_week_number + 1 as the accumulation target
    - This ensures new earnings after a mid-week claim go to the next nonce

  3. Impact
    - Only affects the rare case where a claim happens mid-week and new earnings
      arrive in the same calendar week
    - The 9M6d wallet currently has 2.04 SOL in week 2934 (old trigger) which is
      correct behavior for this scenario -- those earnings arrived after the week
      2933 claim
*/
...
BEGIN
  ...
  IF v_affiliate_wallet IS NOT NULL THEN
    ...
    WHERE affiliate_wallet = v_affiliate_wallet
